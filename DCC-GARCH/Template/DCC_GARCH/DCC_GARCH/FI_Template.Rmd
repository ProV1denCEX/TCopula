...
## Why Gaussian copula does not perform well. 
...
2. Use CIC and cross-validation to select Copula
The best copula should has the largest CIC score.
...
To visualise the copula, we use counter plot on empirical copula on selected two factors(assets).
```{r message=FALSE, warning=FALSE}
...
## Use Copula-DCC-GRACH with different copulas to conduct rolling 1-month forcast
1. Rolling Model
```{r message=FALSE, warning=FALSE}
# reload data
...
for(i in 1:(length(train_period)-1)){
  ...
  for(j in 1:sim_times) scenario_ret_normal[j,] = apply(sim_normal@msim$simX[[j]]+1,2,prod)-1 #simX is simulation return series
  ...
  for(j in 1:sim_times) scenario_ret_std[j,] = apply(sim_std@msim$simX[[j]]+1,2,prod)-1 #simX is simulation return series
